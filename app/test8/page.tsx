...
export default function App(): JSX.Element {
  ...
  useEffect(() => {
    if (selectedGenre || searchQuery) {
      fetchAudioData(selectedGenre, searchQuery, 1);
    }
  }, [selectedGenre, searchQuery]);

  const fetchMoreData = async () => {
    try {
      const apiUrl = `https://api.openverse.engineering/v1/audio/?q=${encodeURIComponent(selectedGenre)}%20${encodeURIComponent(searchQuery)}&page_size=500&page=${page + 1}`;
      ...
      if (data.results && data.results.length > 0) {
        setAudioDetailsList(prevList => [...prevList, ...data.results]);
  ...
  const handleInfiniteScroll = useInfiniteScroll(fetchMoreData);
  ...
  const handleGenreClick = async (genre: string): Promise<void> => {
    ...
    fetchAudioData(genre, searchQuery, 1);
  };

  const fetchAudioData = async (genre: string, query: string, pageNumber: number) => {
    try {
      const apiUrl = `https://api.openverse.engineering/v1/audio/?q=${encodeURIComponent(genre)}%20${encodeURIComponent(query)}&page_size=500&page=${pageNumber}`;
      const response = await fetch(
        apiUrl,
        {
          headers: {
